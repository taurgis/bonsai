...
| Format | Single-quoted string recommended in YAML |
...
| `metadata` | - | Key-value pairs (strings only), including `metadata.version` |
| `allowed-tools` | - | Space-delimited tool patterns |

When version metadata is needed, store it as `metadata.version: '1.0.0'`. Do not add a top-level `version` key to frontmatter.
...
- [ ] If versioned, the skill uses `metadata.version`, not top-level `version`
