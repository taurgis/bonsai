...
normalized_url: 
cache_key: 9a5553e32496f2ba0c838eec2937cbc08774777e4d0a06787869a1b16fa8e8ee
topic: AXI agent-native CLI design principles
tags:
  - axi
  - agentic-ux
  - cli-design
...
ttl: 
fetched_at: 
validated_at: 2026-07-23T13:28:51.351Z
stale_after: 2026-08-22T13:28:51.351Z
capture_method: agent_supplied
extraction_status: agent_supplied
extraction_confidence: high
quality_notes:
  - agent-supplied research import
supplied_at: 2026-07-23T13:28:51.351Z
supplied_by: 
etag: 
last_modified: 
content_hash: edf30ccb3534af3368809d4ba978dd9701802f08a45189c5a4d2ebb30619a8e7
token_estimate:
  compressed: 2986
  detailed: 3369
status: active
...
Synthesized research for AXI agent-native CLI design principles

## Compressed

---
name: axi description: >
...
Errors go to **stdout** in the same structured format as normal output, so the agent can read and act on them. Never let raw dependency output (API errors, stack traces) leak through.
...
Every operation must be completable with flags alone. If a required value is missing, fail immediately with a clear error — don't prompt for it.

### Fail loud on unrecognized input

Reject unknown flags and arguments — never silently ignore them. A dropped flag is worse than an error: the agent gets plausible-looking output it believes is scoped or filtered, then proceeds confidently on wrong data.
...
The session hook is the primary integration, but it only helps agents whose harness supports hooks, and it loads on _every_ session. Recommend the hook first (ambient context plus live state) and the skill second (lower overhead, broader agent support) - they are complementary, and a user installs whichever fits, or both.
...
Imported via agent-supplied research at 2026-07-23T13:28:51.351Z