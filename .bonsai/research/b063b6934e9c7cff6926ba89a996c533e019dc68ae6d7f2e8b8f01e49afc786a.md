...
source_url: https://raw.githubusercontent.com/lirantal/nodejs-cli-apps-best-practices/master/README.md
source_urls:
  - https://raw.githubusercontent.com/lirantal/nodejs-cli-apps-best-practices/master/README.md
normalized_url: https://raw.githubusercontent.com/lirantal/nodejs-cli-apps-best-practices/master/README.md
cache_key: b063b6934e9c7cff6926ba89a996c533e019dc68ae6d7f2e8b8f01e49afc786a
topic: 
tags:
  - cli
  - command
...
etag: 
last_modified: 
content_hash: d6951632febd2e52876966e6b89bd4a48bae2d402ca90f4998660777b9f8ef3b
token_estimate:
  compressed: 13823
  detailed: 17923
status: active
...
A bad CLI can easily discourage users from interacting with it.
...
Unix-like operating systems popularized the use of the command line and tools such as \`awk\`, \`sed\`.
...
Optimize for successful interactions by building empathic CLIs that support the user.
...
A colorful display in your command line application output may further contribute to a richer experience and increased interaction.

For lightweight styling on modern Node.js, prefer \`styleText()\` from \`node:util\`.
...
✅ \*\*Do:\*\* Ensure your program respects \[POSIX signals\] to allow it proper interaction with users or other programs.
...
The problem of not respecting process signals worsens when the program is being orchestrated by non-human interaction.
...
✅ \*\*Do:\*\* Only use prompts and other interactive elements when the command is connected to an interactive terminal.
...
Interactive prompts are useful when a person is present and can answer them. A command should not change from non-interactive to interactive based on ambiguous input.

Use TTY detection, CI detection, and an explicit opt-out such as \`--no-input\` to decide whether prompting is safe.

Example:

\`\`\`js const isInteractive =
  process.stdin.isTTY &&
...
if (!options.projectName) {
  if (isInteractive) {
    ...
    throw new Error(
      'Missing project name. Pass --project-name, set MYCLI\_PROJECT\_NAME, or run in an interactive terminal.'
    );
  }
} \`\`\`
...
- \[Command Line Interface Guidelines: Interactivity\]
...
❌ \*\*Otherwise:\*\* Generic error messages tend to be ambiguous and make it hard for users to search for solutions.

ℹ️ \*\*Details\*\*

Ensure that, when error messages are returned, they include a reference number or specific error codes that can later be consulted.

Example:

\`\`\`sh $ my-cli-tool --doSomething
...
❌ \*\*Otherwise:\*\* Don't skip debugging capabilities.

ℹ️ \*\*Details\*\*

Use environment variables as well as command line arguments to enable extended debug verbosity levels.
...
✅ \*\*Do:\*\* Make it an effortless task to submit bug reports by providing a URL to open an issue and prepopulating the required data as much as possible. \[Issue templates, like on GitHub\], allow to further guide the users as to which information is necessary.
...
As the maintainer of the CLI project, you need to implement a functionality to display the program's version on standard output with the proper flag without any argument, and exit from the program(often also prints compiled-in configuration details as well).

Example:

\`\`\`sh $ my-cli-tool --version

my-cli-tool version 11.0.5, build 11.0.5-0ubuntu1~22.04.1 \`\`\`

### 9.2. Use Semantic Versioning

✅ \*\*Do:\*\* Like other projects, use the SemVer format.
...
❌ \*\*Otherwise:\*\* Users won't know what to expect in new versions, which can lead to frustration or confusion.
...
Argument injection attacks take advantage of vulnerabilities in how command-line applications parse user input.
...
Fetched from https://raw.githubusercontent.com/lirantal/nodejs-cli-apps-best-practices/master/README.md on 2026-07-19T07:01:46.377Z