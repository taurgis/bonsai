...
last_modified: 
content_hash: 444ff3ea63014e01db9a1a62f4d48f1ab6ff2f33af0d5e9f51d2567bbd521924
token_estimate:
  compressed: 14349
  detailed: 15598
status: active
...
Cursor supports loading hooks from third-party tools like Claude Code.
...
The examples below use `./hooks/...` paths, which work for **user hooks** (`~/.cursor/hooks.json`) where scripts run from `~/.cursor/`.
...
Choose TypeScript when you need typed JSON, durable file I/O, and HTTP calls in the same hook.
...
Python shines when you need rich parsing libraries.
...
Define hooks in a `hooks.json` file.
...
Project hooks are the simplest way to share hooks with your team.
...
Distribute hooks across your organization using Mobile Device Management (MDM) tools.
...
Note: MDM-based distribution is fully managed by your organization. Ensure your internal IT or security team handles configuration, deployment, and updates in accordance with your organization's policies.

### Cloud Distribution (Enterprise Only)

Enterprise teams can use Cursor's native cloud distribution to automatically sync hooks to all team members.
...
By default, hook failures (crash, timeout, invalid JSON) allow the action through (fail-open). This is recommended for security-critical `beforeMCPExecution` hooks.
...
By default, `beforeReadFile` hook failures (crash, timeout, invalid JSON) are logged and the read is allowed through.
...
Exit code `2` from command hooks blocks the action (equivalent to returning `permission: "deny"`).
...
Fetched from https://cursor.com/docs/hooks on 2026-06-26T17:04:16.722Z