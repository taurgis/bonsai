...
normalized_url: https://code.claude.com/docs/en/sub-agents
cache_key: 7dc93e85c023d968fea86e6ffdb86488c4659afecd2d96f0dc3238138bb290f5
topic: 
tags:
  - subagent
  - subagents
...
ttl: 
fetched_at: 
validated_at: 2026-06-25T17:16:16.058Z
stale_after: 2026-07-25T17:16:16.058Z
...
supplied_at: 2026-06-25T17:16:16.058Z
supplied_by: 
etag: 
last_modified: 
content_hash: 740187eb5387903f083bfe670b62d8b6e1fa864ea744e05615c06c466dd06731
token_estimate:
  compressed: 14083
  detailed: 17347
status: active
...
Synthesized research for https://code.claude.com/docs/en/sub-agents
...
Built-in subagents are always registered in interactive sessions. To prevent Claude from delegating to any subagent, deny the `Agent` tool itself with [`permissions.deny`].
...
**Project subagents** (`.claude/agents/`) are ideal for subagents specific to a codebase.
...
Claude Code scans `.claude/agents/` and `~/.claude/agents/` recursively, so you can organize definitions into subfolders such as `agents/review/` or `agents/research/`. Keep `name` values unique across the whole tree: if two files within one scope declare the same name, Claude Code keeps one and discards the other without warning.
...
**Managed subagents** are deployed by organization administrators.

**Plugin subagents** come from [plugins] you've installed.
...
The frontmatter defines the subagent's metadata and configuration. Subagents receive only this system prompt (plus basic environment details like working directory), not the full Claude Code system prompt.
...
The following fields can be used in the YAML frontmatter.
...
To restrict tools, use either the `tools` field (allowlist) or the `disallowedTools` field (denylist). The subagent can't edit files, write files, or use any MCP tools:
...
To keep an MCP server out of the main conversation entirely and avoid its tool descriptions consuming context there, define it inline here rather than in `.mcp.json`.
...
Claude Code [passes hook input as JSON] via stdin to hook commands.
...
Define hooks directly in the subagent's markdown file.
...
When automatic delegation isn't enough, you can request a subagent yourself.
...
Subagents provided by an enabled [plugin] appear in the typeahead under their scoped name, such as `my-plugin:code-reviewer` or `my-plugin:review:security` when the plugin [organizes agents into subfolders].
...
The subagent's system prompt replaces the default Claude Code system prompt entirely, the same way [`--system-prompt`] does. `CLAUDE.md` files and project memory still load through the normal message flow.
...
Claude decides whether to run subagents in the foreground or background based on the task.
...
One of the most effective uses for subagents is isolating operations that produce large amounts of output. By delegating these to a subagent, the verbose output stays in the subagent's context while only the relevant summary returns to your main conversation.
...
{/* min-version: 2.1.172 */}As of Claude Code v2.1.172, a subagent can spawn its own subagents. Use this when a delegated task itself splits into parallel subtasks, such as a reviewer subagent that dispatches a verifier per finding, so the intermediate output never reaches your main conversation.

A nested subagent is configured the same way as a top-level one and resolves from the same [scopes].

Depth is counted as the number of subagent levels below the main conversation, regardless of whether each level runs in the [foreground or background]. A subagent at depth five does not receive the Agent tool and cannot spawn further.

As of Claude Code v2.1.187, a background subagent's depth is fixed when it is first spawned, and [resuming] it later does not change that depth. For example, if your main conversation spawns subagent A, and A spawns a background subagent B at depth two, B is still at depth two when you resume it directly from the main conversation.

To prevent a specific subagent from spawning others, omit `Agent` from its [`tools`] list or add it to `disallowedTools`.

A [fork] still cannot spawn another fork.

### Manage subagent context

#### What loads at startup

Each subagent starts with a fresh, isolated context window. Claude composes a delegation message that summarizes the task, and the subagent works from there.
...
The main conversation reads Explore and Plan results with full CLAUDE.md context, so most rules don't need to reach the subagent itself.
...
You can also ask Claude for the agent ID if you want to reference it explicitly, or find IDs in the transcript files at `~/.claude/projects/{project}/{sessionId}/subagents/`.
...
Subagents support automatic compaction using the same logic as the main conversation.
...
<Note>
  Forked subagents require Claude Code v2.1.117 or later. {/* min-version: 2.1.161 */}From v2.1.161 the `/fork` command is enabled by default; on earlier versions it requires setting the [`CLAUDE_CODE_FORK_SUBAGENT`] environment variable to `1`. Letting Claude itself spawn forks is experimental and may change in future releases. This capability may also be enabled in interactive sessions as part of a staged rollout.
</Note>
...
* Claude can spawn a fork by requesting the `fork` subagent type explicitly. Spawns without a subagent type still use the [general-purpose] subagent, and named subagents such as Explore still spawn as before.
...
A fork inherits everything the main session has at the moment it spawns.
...
Because a fork's system prompt and tool definitions are identical to the parent, its first request reuses the parent's [prompt cache].
...
These examples demonstrate effective patterns for building subagents.
...
Create the validation script anywhere in your project.
...
The hook receives JSON via stdin with the Bash command in `tool_input.command`.
...
Imported via agent-supplied research at 2026-06-25T17:16:16.058Z