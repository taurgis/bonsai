...
normalized_url: https://llmstxt.org/
cache_key: 6e6f293d07aa413e876c343b925649b6039d01075a10fd2e69de8a53ae3509ba
topic: 
tags:
  - file
  - information
...
tier: standard
ttl: 
fetched_at: 2026-06-26T20:30:31.150Z
validated_at: 2026-06-26T20:30:31.150Z
stale_after: 2026-07-26T20:30:31.150Z
capture_method: route_markdown
extraction_status: extracted
extraction_confidence: high
quality_notes:
  - captured from public Markdown/MDX source: https://llmstxt.org/index.md
  - auto-generated tags via keyword extraction
supplied_at: 
supplied_by: 
etag: 
last_modified: Tue, 09 Jun 2026 14:27:50 GMT
content_hash: 3fff82fce8f361e908ace3b06fe6c4dd4ad7075300fedce1c5c5de9c38693fa7
token_estimate:
  compressed: 1996
  detailed: 2768
status: active
...
docs_framework: 
source_doc_url: https://llmstxt.org/index.md
search_provider: 
parent_cache_key: 
section_anchor: 
...
The [FastHTML project] follows these two proposals for its documentation.

This proposal does not include any particular recommendation for how to process the llms.txt file, since it will depend on the application. For example, the FastHTML project opted to automatically expand the llms.txt to two markdown files with the contents of the linked URLs, using an XML-based structure suitable for use in LLMs such as Claude. They are created using the [`llms_txt2ctx`] command line application, and the FastHTML documentation includes information for users about how to use them.
...
Note that all [nbdev] projects now create .md versions of all pages by default.

## Format

At the moment the most widely and easily understood format for language models is Markdown.

The `llms.txt` file is unusual in that it uses Markdown to structure the information rather than a classic structured format such as XML. Having said that, the information in llms.txt follows a specific format and can be read using standard programmatic-based tools.
...
- Zero or more markdown sections delimited by H2 headers, containing
  ...
  - Each “file list” is a markdown list, containing a required markdown
    hyperlink `[name]`, then optionally a `:` and notes about the
    file.
...
Note that the “Optional” section has a special meaning—if it’s included, the URLs provided there can be skipped if a shorter context is needed.

## Existing standards

llms.txt is designed to coexist with current web standards. The file can also reference structured data markup used on the site, helping LLMs understand how to interpret this information in context.

The approach of standardising on a path for the file follows the approach of `/robots.txt` and `/sitemap.xml`. robots.txt and `llms.txt` have different purposes—robots.txt is generally used to let automated tools know what access to a site is considered acceptable, such as for search indexing bots. On the other hand, `llms.txt` information will often be used on demand when a user explicitly requests information about a topic, such as when including a coding library’s documentation in a project, or when asking a chat bot with search functionality for information. However, perhaps if `llms.txt` usage becomes widespread, future training runs could take advantage of the information in `llms.txt` files too.
...
- [JavaScript Implementation] - Sample JavaScript
  implementation
...
The `llms.txt` specification is open for community input.
...
Fetched from https://llmstxt.org/ on 2026-06-26T20:30:31.150Z