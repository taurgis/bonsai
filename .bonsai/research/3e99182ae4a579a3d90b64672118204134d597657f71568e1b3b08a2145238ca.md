...
source_url: https://code.visualstudio.com/docs/agent-customization/custom-agents
source_urls:
  - https://code.visualstudio.com/docs/agent-customization/custom-agents
normalized_url: https://code.visualstudio.com/docs/agent-customization/custom-agents
cache_key: 3e99182ae4a579a3d90b64672118204134d597657f71568e1b3b08a2145238ca
topic: 
tags:
  - agent
  - custom
...
tier: standard
ttl: 
fetched_at: 2026-07-19T07:45:09.336Z
validated_at: 2026-07-19T07:45:09.336Z
stale_after: 2026-08-18T07:45:09.336Z
...
supplied_at: 
supplied_by: 
etag: W/"19520-2PCU5JtbG1Z56H2hxGWok27VCBQ"
last_modified: 
content_hash: ed0b1c68a2bbb05a9f11cb78924ee305fdfd1f799661e785f5d1af8456a5fdda
token_estimate:
  compressed: 3943
  detailed: 5173
status: active
...
Custom agents enable you to configure the AI to adopt different personas tailored to specific development roles and tasks. For example, you might create agents for a security reviewer, planner, solution architect, or other specialized roles.

You can also use handoffs to create guided workflows between agents.
...
Custom agents consist of a set of instructions and tools that are applied when you switch to that agent. By creating a custom agent, you can quickly switch to that specific configuration without having to manually select relevant tools and instructions each time.
...
Handoffs are useful for orchestrating multi-step workflows that give developers control for reviewing and approving each step before moving to the next one.
...
To define handoffs in your agent file, add them to the frontmatter.
...
In a monorepo, enable chat.useCustomizationsInParentRepositories to discover custom agents from the parent repository root.
...
The custom agent file body contains the custom agent implementation, formatted as Markdown.
...
The following code snippet shows an example of a "Plan" custom agent file that generates an implementation plan and doesn't make any code edits.
...
The following example shows a "Feature Builder" agent that coordinates specialized subagents for a research-then-implement workflow.
...
Agent with scoped hooks example (Preview)

The following example shows a custom agent that defines hooks in its frontmatter.
...
2.  Select **New Agent (Workspace)** or **New Agent (User)** from the dropdown, depending on where you want to store the agent file.
    ...
    Alternatively, run the **Chat: New Custom Agent** command from the Command Palette (⇧⌘P (Windows, Linux Shift+cmd+P)).
...
You can use AI to generate a custom agent based on a description of the role. Type `/create-agent` in Agent mode chat and describe the persona you want (for example, "a security review agent").

You can also extract a custom agent from an ongoing conversation.
...
If you have multiple custom agents, you can customize which ones appear in the agents dropdown.
...
When you use `tools` in both a custom agent and a prompt file, the prompt file's tools take precedence.

To share custom agents across your team, you can create a workspace-level custom agent (`.github/agents` folder).

VS Code automatically detects custom agents defined at the organization level to which your account has access.
...
If you have existing `.chatmode.md` files, rename them to `.agent.md` to convert them to the new custom agent format and place them in the appropriate location ( chat.agentFilesLocations ) to continue using them.
...
Use the chat customization diagnostics view to see all loaded custom agents, prompt files, instruction files, and skills along with any errors.

## Security considerations

Custom agents can restrict which tools are available, which gives you control over what the AI can do. For security-sensitive workflows, create agents with read-only tools to prevent unintended modifications.
...
**Not sure which customization to use?** See the [decision matrix](https://code.visualstudio.com/docs/agents/concepts/customization#_customization-options-at-a-glance) to compare custom agents with prompt files, agent skills, and the other options.
...
The [built-in agents](https://code.visualstudio.com/docs/agents/agent-types/local-agents) provide general-purpose configurations for chat in VS Code. For a more tailored chat experience, you can create your own custom agents.
...
You can reuse your custom agents in [background agents](https://code.visualstudio.com/docs/agents/agent-types/copilot-cli) and [cloud agents](https://code.visualstudio.com/docs/agents/agent-types/cloud-agents), enabling you to run autonomous tasks with the same specialized configurations.
...
In a monorepo, enable chat.useCustomizationsInParentRepositories to discover custom agents from the parent repository root. Learn more about [parent repository discovery](https://code.visualstudio.com/docs/agent-customization/overview#_use-customizations-in-a-monorepo).
...
The following example shows a custom agent that defines hooks in its frontmatter. The `PostToolUse` hook runs a formatter after file edits and only runs when this agent is active. Enable chat.useCustomAgentHooks to use this feature.
...
Learn more about hooks in [Agent hooks](https://code.visualstudio.com/docs/agent-customization/hooks).
...
2.  Select **New Agent (Workspace)** or **New Agent (User)** from the dropdown, depending on where you want to store the agent file.
    ...
    ![Screenshot of the Agent Customizations editor, showing the Agents tab and the dropdown to create a new custom agent.](/assets/docs/agent-customization/customization/create-custom-agent.png)
...
When you use `tools` in both a custom agent and a prompt file, the prompt file's tools take precedence. For the full priority order, see [Tool list priority](https://code.visualstudio.com/docs/agent-customization/prompt-files#_tool-list-priority) in the prompt files documentation.
...
To remove a custom agent that was contributed by an extension, you need to uninstall the extension that provides it. If you don't want to uninstall the extension, you can hide the custom agent from the agents dropdown instead. Follow the steps in [Customize the agents dropdown list](https://code.visualstudio.com/docs/agent-customization/custom-agents#_customize-the-agents-dropdown-list).
...
Use the chat customization diagnostics view to see all loaded custom agents, prompt files, instruction files, and skills along with any errors. Right-click in the Chat view and select **Diagnostics**. Learn more about [troubleshooting AI in VS Code](https://code.visualstudio.com/docs/agents/agent-troubleshooting/troubleshooting).
...
*   [Customize AI with custom instructions](https://code.visualstudio.com/docs/agent-customization/custom-instructions)
*   [Create reusable prompt files](https://code.visualstudio.com/docs/agent-customization/prompt-files)
...
Fetched from https://code.visualstudio.com/docs/agent-customization/custom-agents on 2026-07-19T07:45:09.336Z