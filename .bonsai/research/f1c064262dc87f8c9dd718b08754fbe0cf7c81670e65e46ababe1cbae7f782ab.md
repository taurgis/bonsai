...
cache_key: f1c064262dc87f8c9dd718b08754fbe0cf7c81670e65e46ababe1cbae7f782ab
topic: 
tags:
  - files
  - claude
...
etag: 
last_modified: 
content_hash: 10c03933785bf8548ac03026e6fa08ed4e9fc75dff083ef7b31fb1604144ac0b
token_estimate:
  compressed: 6131
  detailed: 8166
status: active
...
**Size**: target under 200 lines per CLAUDE.md file. If your instructions are growing large, use [path-scoped rules] so instructions load only when Claude works with matching files.
...
Claude Code reads `CLAUDE.md`, not `AGENTS.md`. Claude loads the imported file at session start, then appends the rest:
...
{/* min-version: 2.1.210 */}After Claude writes to `MEMORY.md`, Claude Code measures the file against the 200-line and 25KB read limits. If the file is near a limit, Claude Code reminds Claude to shorten it: keep one line per entry, move detail into topic files, and merge or drop stale entries.
...
The `/memory` command lists your CLAUDE.md, CLAUDE.local.md, and other memory file locations across user and project scopes, lets you toggle auto memory on or off, and provides an option to open the auto memory folder.
...
Files over 200 lines consume more context and may reduce adherence. Use [path-scoped rules] to load instructions only when Claude works with matching files, or trim content that isn't needed in every session.
...
Fetched from https://code.claude.com/docs/en/memory on 2026-07-19T13:32:46.158Z