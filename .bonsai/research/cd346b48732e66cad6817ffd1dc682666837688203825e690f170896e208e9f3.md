...
source_url: https://bonsai.rhino-inquisitor.com/how-to/agent-kit
source_urls:
  - https://bonsai.rhino-inquisitor.com/how-to/agent-kit
  - https://raw.githubusercontent.com/taurgis/bonsai/main/docs/how-to/agent-kit.md
normalized_url: https://bonsai.rhino-inquisitor.com/how-to/agent-kit
cache_key: cd346b48732e66cad6817ffd1dc682666837688203825e690f170896e208e9f3
topic: 
tags:
  - agent
  - subagent
...
tier: standard
ttl: 
fetched_at: 2026-06-26T20:44:00.228Z
validated_at: 2026-06-26T20:44:00.228Z
stale_after: 2026-07-26T20:44:00.228Z
capture_method: github_source
extraction_status: extracted
extraction_confidence: high
quality_notes:
  - captured from public Markdown/MDX source: https://raw.githubusercontent.com/taurgis/bonsai/main/docs/how-to/agent-kit.md
  - auto-generated tags via keyword extraction
supplied_at: 
supplied_by: 
etag: 
last_modified: Fri, 26 Jun 2026 20:41:16 GMT
content_hash: 655ad994549b0aa6adcce17e7d04313e756e174e5ade54a2419c77d337bf1b0a
token_estimate:
  compressed: 2687
  detailed: 3552
status: active
...
docs_framework: 
source_doc_url: https://raw.githubusercontent.com/taurgis/bonsai/main/docs/how-to/agent-kit.md
...
Bonsai ships a small kit that teaches an AI coding agent to research the cache-first way: search what's already captured, fetch through Bonsai when the cache misses, and import anything it had to read by hand. The core kit is three files — a skill, an instruction, and a subagent — plus an optional Salesforce variant, and you install it into your own agent's config.
...
The skill is the engine, the instruction decides when it fires, and the subagent isolates the heavy research.

### Optional: Salesforce variant

The kit also ships a Salesforce specialization of the instruction and subagent, for teams whose research targets Salesforce docs.
...
- [View the instruction source →]
- [View the subagent source →]

`--all` installs these alongside the generic pieces; omit them if you don't research Salesforce.

## Install with forward-nexus

[`forward-nexus`] reads the kit from the repo and writes it into your project in the right format.
...
Each target writes to that agent's own config directory (`.github/`, `.claude/`, `.cursor/`).
...
Copy the files into the matching locations in your project.
...
The hooks do not silently rewrite a web fetch into a shell command.

```bash
bonsai <url> --format detailed
```

Use `node bin/cli.mjs <url> --format detailed` when testing inside the Bonsai repo.

These examples are based on the current native hook docs for [Claude Code], [GitHub Copilot], [Cursor], and [Codex].
...
That is enough for most junior-friendly workflows.
...
The agent does not need to browse first and cache later.

## What lands for Claude Code

`--agent=claude-code` writes three files, each a native Claude Code config.
...
Claude Code loads every `.md` file under [`.claude/rules/`] at launch, with the same weight as `CLAUDE.md`.
...
- Instruction — [`agents/instructions/web-research.instructions.md`]
  (`applyTo: '**'`, with a `skills:` link to the skill)
- Subagent — [`agents/agents/official-docs-researcher.agent.md`]
...
- Rule — [`.cursor/rules/web-research.mdc`]

Cursor has no documented project-level _subagent_ file.

## Verify it works

After installing, the agent should reach for Bonsai before fetching.
...
Fetched from https://bonsai.rhino-inquisitor.com/how-to/agent-kit on 2026-06-26T20:44:00.228Z