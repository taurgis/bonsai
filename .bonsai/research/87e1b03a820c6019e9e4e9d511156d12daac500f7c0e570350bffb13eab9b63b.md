...
source_url: https://code.visualstudio.com/docs/agent-customization/custom-instructions
source_urls:
  - https://code.visualstudio.com/docs/agent-customization/custom-instructions
normalized_url: https://code.visualstudio.com/docs/agent-customization/custom-instructions
cache_key: 87e1b03a820c6019e9e4e9d511156d12daac500f7c0e570350bffb13eab9b63b
topic: 
tags:
  - instructions
  - file
...
tier: standard
ttl: 
fetched_at: 2026-07-19T07:01:51.861Z
validated_at: 2026-07-19T07:01:51.861Z
stale_after: 2026-08-18T07:01:51.861Z
...
supplied_at: 
supplied_by: 
etag: W/"1c0bb-DsmY2l4tIevjay0n6/qlkQFKq2k"
last_modified: 
content_hash: 04ea4313c441a5470553b5b7bd0f51901a8fe2d1e77d336169cbfed4735840bd
token_estimate:
  compressed: 4414
  detailed: 5915
status: active
...
Use the [Agent Customizations editor] (Preview) to discover, create, and manage all your agent customizations in one place.
...
VS Code supports two categories of custom instructions.

### Always-on instructions

Always-on instructions are automatically included in every chat request.
...
File-based instructions are applied when files that the agent is working on match a specified pattern or if the description matches the current task.
...
**Which approach should you use?** Start with a single `.github/copilot-instructions.md` file for project-wide coding standards.
...
You can define instructions for a specific workspace or at the user level, where they are applied across all your workspaces. You can configure additional file locations for workspace instructions files with the chat.instructionsFilesLocations setting.
...
VS Code searches these folders recursively, to enable you to organize instructions files in subdirectories.
...
In a monorepo, enable chat.useCustomizationsInParentRepositories to discover instructions from the parent repository root.
...
When you create an instructions file, choose whether to store it in your workspace or user profile.
...
2.  Select **New Instructions (Workspace)** or **New Instructions (User)** from the dropdown, depending on where you want to store the instructions file.
    
    
    
    Alternatively, use the **Chat: New Instructions File** command from the Command Palette (⇧⌘P (Windows, Linux Shift+cmd+P)).
    
3.  Select the location and enter a file name for your instructions file. This is the default name that is used in the UI.
...
You can also extract instructions from an ongoing conversation.

Note

`/create-instruction` generates targeted, on-demand instruction files.

Example: Language-specific coding guidelines

Notice how these instructions reference the general coding guidelines file.
...
Using multiple `AGENTS.md` files in subfolders is useful if you want to apply different instructions to different parts of your project.
...
When enabled, VS Code searches recursively in all subfolders of your workspace for `AGENTS.md` files and adds their relative path to the chat context.
...
VS Code automatically detects a `CLAUDE.md` file and applies it as always-on instructions, similar to `AGENTS.md`.
...
For `.claude/rules` instructions files, VS Code uses a `paths` property instead of `applyTo` for glob patterns, following the [Claude Rules format].
...
2.  It analyzes your project structure and coding patterns.
3.  It generates comprehensive workspace instructions tailored to your project.
...
VS Code automatically detects custom instructions defined at the organization level to which your account has access.
...
To sync your user instructions files, enable Settings Sync and run **Settings Sync: Configure** from the Command Palette (⇧⌘P (Windows, Linux Shift+cmd+P)).

## Specify custom instructions in settings

Note

Settings-based code generation and test generation instructions are deprecated as of VS Code 1.102.

For code review, commit messages, and pull request descriptions, you can still use VS Code settings to define custom instructions.
...
When multiple types of custom instructions exist, they are all provided to the AI.
...
Use the chat customization diagnostics view to see all loaded instruction files and any errors.
...
1.  Select **Chat: Configure Instructions** from the Command Palette (⇧⌘P (Windows, Linux Shift+cmd+P)).
2.  Hover over the instruction file in the list. The source location is displayed in a tooltip.
...
*   A single [`.github/copilot-instructions.md`](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_use-a-githubcopilot-instructionsmd-file) file
    
    *   Automatically applies to all chat requests in the workspace
    *   Stored within the workspace
*   One or more [`AGENTS.md`](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_use-an-agentsmd-file) files
    
    *   Useful if you work with multiple AI agents in your workspace
    *   Automatically applies to all chat requests in the workspace or to specific subfolders (experimental)
    *   Stored in the root of the workspace or in subfolders (experimental)
*   [Organization-level instructions](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_share-custom-instructions-across-teams)
    
    *   Share instructions across multiple workspaces and repositories within a GitHub organization
    *   Defined at the GitHub organization level
*   [`CLAUDE.md`](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_use-a-claudemd-file) file
...
*   One or more [`.instructions.md`](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_use-instructionsmd-files) files
    *   Conditionally apply instructions based on file type or location by using glob patterns
    *   Stored in the workspace or user profile
...
VS Code also supports the use of an [`AGENTS.md` file](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_use-an-agentsmd-file) for always-on instructions.
...
You can define instructions for a specific workspace or at the user level, where they are applied across all your workspaces. The following table lists the default file locations for instructions files based on their scope. You can configure additional file locations for workspace instructions files with the chat.instructionsFilesLocations setting.
...
In a monorepo, enable chat.useCustomizationsInParentRepositories to discover instructions from the parent repository root. Learn more about [parent repository discovery](https://code.visualstudio.com/docs/agent-customization/overview#_use-customizations-in-a-monorepo).
...
2.  Select **New Instructions (Workspace)** or **New Instructions (User)** from the dropdown, depending on where you want to store the instructions file.
    ...
    ![Screenshot of the Agent Customizations editor, showing the Instructions tab and the dropdown to create a new instructions file.](/assets/docs/agent-customization/customization/create-instructions-file.png)
...
`/create-instruction` generates targeted, on-demand instruction files. To generate workspace-wide always-on instructions, use the [`/init` command](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_generate-custom-instructions-for-your-workspace) instead.
...
For folder-specific instructions, you can also use multiple [`.instructions.md`](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_use-instructionsmd-files) files with different `applyTo` patterns that match the folder structure.
...
VS Code can sync your user instructions files across multiple devices by using [Settings Sync](https://code.visualstudio.com/docs/configure/settings-sync).

To sync your user instructions files, enable Settings Sync and run **Settings Sync: Configure** from the Command Palette (⇧⌘P (Windows, Linux Shift+cmd+P)). Select **Prompts and Instructions** from the list of settings to sync.

## Specify custom instructions in settings

Note

Settings-based code generation and test generation instructions are deprecated as of VS Code 1.102. Use [file-based instructions](https://code.visualstudio.com/docs/agent-customization/custom-instructions#_types-of-instruction-files) instead.
...
*   Reuse and reference instructions files in your [prompt files](https://code.visualstudio.com/docs/agent-customization/prompt-files) and [custom agents](https://code.visualstudio.com/docs/agent-customization/custom-agents) to keep them clean and focused, and to avoid duplicating instructions.
...
Use the chat customization diagnostics view to see all loaded instruction files and any errors. Right-click in the Chat view and select **Diagnostics**. Learn more about [troubleshooting AI in VS Code](https://code.visualstudio.com/docs/agents/agent-troubleshooting/troubleshooting).
...
*   [Use Agent Skills](https://code.visualstudio.com/docs/agent-customization/agent-skills)
*   [Create custom agents](https://code.visualstudio.com/docs/agent-customization/custom-agents)
...
Fetched from https://code.visualstudio.com/docs/agent-customization/custom-instructions on 2026-07-19T07:01:51.861Z