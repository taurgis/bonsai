...
normalized_url: https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html
cache_key: c35c03d3541d52dc0bff0d0d9bdfced885668433670001e953a88f17cd8dbb5e
topic: 
tags:
  - validation
  - input
...
tier: standard
ttl: 
fetched_at: 2026-07-19T10:01:32.419Z
validated_at: 2026-07-19T10:01:32.419Z
stale_after: 2026-08-18T10:01:32.419Z
...
content_hash: 4f83f1d766807726de66233d7fd29cf938dcf12bdc5555642136da01920cde1c
token_estimate:
  compressed: 3695
  detailed: 5688
status: active
...
Input Validation should not be used as the _primary_ method of preventing [XSS], [SQL Injection] and other attacks which are covered in respective [cheat sheets] but can significantly contribute to reducing their impact if implemented properly.
...
*   Use data type validators available natively in web application frameworks (such as [Django Validators], [Apache Commons Validators] etc).
...
Plus, such filters frequently prevent authorized input, like `O'Brian`, where the `'` character is fully legitimate.

While denylisting can be useful as an additional layer of defense to catch some common malicious patterns, it should not be relied upon as the primary method.
...
When designing regular expressions, be aware of [RegEx Denial of Service (ReDoS) attacks].
...
Some Allowlist validators have also been predefined in various open source packages that you can leverage.
...
All user data controlled must be encoded when returned in the HTML page to prevent the execution of malicious data (e.g.
...
Many websites allow users to upload files, such as a profile picture or more.
...
The biggest caveat on this is that although the RFC defines a very flexible format for email addresses, most real world implementations (such as mail servers) use a far more restricted address format, meaning that they will reject addresses that are _technically_ valid.

As such, the best way to validate email addresses is to perform some basic initial validation, and then pass the address to the mail server and catch the exception if it rejects it.
...
Semantic validation is about determining whether the email address is correct and legitimate.
...
Blocking disposable email addresses is almost impossible, as there are a large number of websites offering these services, with new domains being created every day.
...
If it is essential that disposable email addresses are blocked, then registrations should only be allowed from specifically-allowed email providers.

#### Sub-Addressing[¶]

Sub-addressing allows a user to specify a _tag_ in the local part of the email address (before the `@` sign), which will be ignored by the mail server.

*   `[[email protected]]`
*   `[[email protected]]`
*   `[[email protected]]`

Many mail providers (such as Microsoft Exchange) do not support sub-addressing.
...
Because it could allow users to register multiple accounts with a single email address, some sites may wish to block sub-addressing by stripping out everything between the `+` and `@` signs.
...
## Introduction[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#introduction "Permanent link")

This article is focused on providing clear, simple, actionable guidance for providing Input Validation security functionality in your applications.

## Goals of Input Validation[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#goals-of-input-validation "Permanent link")
...
Input Validation should not be used as the _primary_ method of preventing [XSS](https://cheatsheetseries.owasp.org/cheatsheets/Cross_Site_Scripting_Prevention_Cheat_Sheet.html), [SQL Injection](https://cheatsheetseries.owasp.org/cheatsheets/SQL_Injection_Prevention_Cheat_Sheet.html) and other attacks which are covered in respective [cheat sheets](https://cheatsheetseries.owasp.org/) but can significantly contribute to reducing their impact if implemented properly.

## Input Validation Strategies[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#input-validation-strategies "Permanent link")
...
## Implementing Input Validation[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#implementing-input-validation "Permanent link")
...
### Allowlist vs Denylist[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#allowlist-vs-denylist "Permanent link")
...
### Validating Free-form Unicode Text[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#validating-free-form-unicode-text "Permanent link")
...
### Regular Expressions (Regex)[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#regular-expressions-regex "Permanent link")
...
## Allowlist Regular Expression Examples[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#allowlist-regular-expression-examples "Permanent link")
...
## Client-side vs Server-side Validation[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#client-side-vs-server-side-validation "Permanent link")

Input validation **must** be implemented on the server-side before any data is processed by an application’s functions, as any JavaScript-based input validation performed on the client-side can be circumvented by an attacker who disables JavaScript or uses a web proxy. Implementing both client-side JavaScript-based validation for UX and server-side validation for security is the recommended approach, leveraging each for their respective strengths.

## Validating Rich User Content[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#validating-rich-user-content "Permanent link")
...
## Preventing XSS and Content Security Policy[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#preventing-xss-and-content-security-policy "Permanent link")
...
## File Upload Validation[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#file-upload-validation "Permanent link")
...
### Upload Verification[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#upload-verification "Permanent link")

*   Use input validation to ensure the uploaded filename uses an expected extension type.
*   Ensure the uploaded file is not larger than a defined maximum file size.
*   If the website supports ZIP file upload, do a validation check before unzipping the file. The check includes the target path, level of compression, estimated unzip size.

### Upload Storage[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#upload-storage "Permanent link")
...
### Public Serving of Uploaded Content[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#public-serving-of-uploaded-content "Permanent link")

*   Ensure uploaded images are served with the correct content-type (e.g. `image/jpeg`, `image/png`)

### Beware of Specific File Types[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#beware-of-specific-file-types "Permanent link")
...
### Image Upload Verification[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#image-upload-verification "Permanent link")

*   Use image rewriting libraries to verify the image is valid and to strip away extraneous content.
*   Set the extension of the stored image to be a valid image extension based on the detected content type of the image from image processing (e.g. do not just trust the header from the upload).
*   Ensure the detected content type of the image is within a list of defined image types (jpg, PNG, etc)

## Email Address Validation[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#email-address-validation "Permanent link")

### Syntactic Validation[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#syntactic-validation "Permanent link")
...
*   `"><script>alert(1);</script>"@example.org`
*   `[[email protected]](https://cheatsheetseries.owasp.org/cdn-cgi/l/email-protection)`
*   `user@[IPv6:2001:db8::1]`
*   `" "@example.org`
...
### Semantic Validation[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#semantic-validation "Permanent link")
...
#### Disposable Email Addresses[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#disposable-email-addresses "Permanent link")
...
#### Sub-Addressing[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#sub-addressing "Permanent link")
...
Because it could allow users to register multiple accounts with a single email address, some sites may wish to block sub-addressing by stripping out everything between the `+` and `@` signs. This is not generally recommended, as it suggests that the website owner is either unaware of sub-addressing or wishes to prevent users from identifying them when they leak or sell email addresses. Additionally, it can be trivially bypassed by using [disposable email addresses](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#disposable-email-addresses), or simply registering multiple email accounts with a trusted provider.

## References[¶](https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html#references "Permanent link")
...
Fetched from https://cheatsheetseries.owasp.org/cheatsheets/Input_Validation_Cheat_Sheet.html on 2026-07-19T10:01:32.419Z