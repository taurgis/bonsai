...
source_url: https://vitepress.dev/reference/site-config
source_urls:
  - https://vitepress.dev/reference/site-config
  - https://vitepress.dev/reference/site-config.md
normalized_url: https://vitepress.dev/reference/site-config
cache_key: 93645f7995021d71010cde0093b1134acf982a543bd31682c88c3947590077a1
topic: 
tags:
  - config
  - type
...
tier: standard
ttl: 
fetched_at: 2026-06-26T20:34:15.133Z
validated_at: 2026-06-26T20:34:15.133Z
stale_after: 2026-07-26T20:34:15.133Z
capture_method: route_markdown
extraction_status: extracted
extraction_confidence: high
quality_notes:
  - captured from public Markdown/MDX source: https://vitepress.dev/reference/site-config.md
  - auto-generated tags via keyword extraction
supplied_at: 
supplied_by: 
etag: "345da6a0fef33d9a532532a6688728bc-ssl-df"
last_modified: 
content_hash: fd7f0127ac949b6a693119d8046bc20dbf7a7387e5d72934d5f8f145a9368eb6
token_estimate:
  compressed: 4224
  detailed: 5291
status: active
...
docs_framework: 
source_doc_url: https://vitepress.dev/reference/site-config.md
...
Site config is where you can define the global settings of the site. App config options define settings that apply to every VitePress site, regardless of what theme it is using.

## Overview

### Config Resolution

The config file is always resolved from `<root>/.vitepress/config.[ext]`, where `<root>` is your VitePress [project root], and `[ext]` is one of the supported file extensions.
...
If you need to dynamically generate the config, you can also default export a function.
...
This is achieved by adding a file called `config.ts` (or `.js`, `.mjs`, or `.mts`) in the relevant directory.
...
For example, for a site with multiple languages we might want a different `description` for each language.
...
Alternatively, when using the built-in i18n features, the settings for a locale directory can be overridden via the `locales` setting in the main configuration file.
...
* Type: `string`
* Default: `VitePress`
* Can be overridden per page via [frontmatter] or at the [directory level]

Title for the site.

It will also be used as the default suffix for all individual page titles, unless [`titleTemplate`] is defined. For example with the following config and page content:
...
The lang attribute for the site.
...
::: warning Server Support Required Enabling this may require additional configuration on your hosting platform.
...
Specify the directory to nest generated assets under.
...
When set to `true`, extract pages metadata to a separate JavaScript chunk instead of inlining it in the initial HTML.
...
When set to `true`, the production app will be built in [MPA Mode].
...
This option injects an inline script that restores users settings from local storage using the `vitepress-theme-appearance` key.
...
When using the default theme, enabling this option will display each page's last updated time.
...
`transformHead` is a build hook to add extra tags to the `<head>` of each page. It allows you to add head entries that cannot be statically added to your VitePress config.
...
The extra tags will be added to the static HTML files generated by the build.

In many cases, using the [`transformPageData`] hook is a cleaner solution.
...
Here we're assuming that the image URL is dynamic and time-consuming to generate.
...
::: warning Don't mutate anything inside the `context` and be careful that this might impact the performance of dev server, especially if you have some network requests or heavy computations (like generating images) in the hook.
...
Fetched from https://vitepress.dev/reference/site-config on 2026-06-26T20:34:15.133Z