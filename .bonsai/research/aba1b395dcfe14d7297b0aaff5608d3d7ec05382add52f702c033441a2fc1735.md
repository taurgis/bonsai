...
cache_key: aba1b395dcfe14d7297b0aaff5608d3d7ec05382add52f702c033441a2fc1735
topic: 
tags:
  - codex
  - hooks
...
content_hash: cf4cbcbfa825e413e429863488cf9c4205f8b948410a364bf2537a1863af6e76
token_estimate:
  compressed: 5377
  detailed: 6844
status: active
...
Codex can also discover hooks bundled with enabled plugins.

Project-local hooks load only when the project `.codex/` layer is trusted.

Codex lists configured hooks before deciding which ones can run. Before a non-managed command hook can run, Codex requires you to review and trust the exact hook definition.
...
Manifest hook paths are resolved relative to the plugin root and must stay inside that root.
...
Plugin hooks use the same event schema as other hooks.
...
`PreToolUse` can intercept Bash, file edits performed through `apply_patch`, and MCP tool calls.
...
JSON on `stdout` can use `systemMessage`.
...
`permissionDecision: "ask"`, legacy `decision: "approve"`, `continue: false`, `stopReason`, and `suppressOutput` are parsed but not supported yet.
...
To stop normal processing of the original tool result after the command has already run, return `continue: false`.
...
Installed plugins can also bundle lifecycle config through their plugin manifest or a default `hooks/hooks.json` file. See [Build plugins](https://developers.openai.com/codex/plugins/build#bundled-mcp-servers-and-lifecycle-hooks) for the plugin packaging rules.
...
Fetched from https://developers.openai.com/codex/hooks on 2026-06-26T17:00:36.155Z