...
- **Obtain a Client Credentials grant:** in a request directed to the *Digital Authorization server*, the *Client ID* and *client password* are included in the *Authorization* header as required by the [HTTP Basic Authentication mechanism (RFC 2617)](http://www.ietf.org/rfc/rfc2617.txt). (The client ID and client password, joined by a ':' are encoded using the base-64 encoding scheme.) The parameter *grant_type* is required and must be set to 'client_credentials'. See the sample below:
...
- **Obtain a Authorization Code Grant:**
  Login into the Digital Authorization server with the following request:
