...
Bonsai is not only for pages it fetched. An agent can write its own
research, a synthesis stitched together from several sources, a cleaned-up
extract, notes from a page behind a login, and store it in the same cache as
everything else. The next session then looks it up exactly like a fetched page.
The `import` command is how that text gets in.
...
[`search`](/guide/search), filtering by `--topic` or `--artifact-type research_note`.
...
[Storage Modes](/guide/storage-modes) for the project-versus-global split and
